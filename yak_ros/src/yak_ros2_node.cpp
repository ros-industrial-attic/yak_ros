...
public:
   ...
   */
  explicit Fusion(const rclcpp::Node::SharedPtr node, const kfusion::KinFuParams& params, const std::string tsdf_base_frame) :
    node_(node),
    ...
    tsdf_base_frame_(tsdf_base_frame)
  {
    ...
    generate_mesh_service_ = node_->create_service<std_srvs::srv::Trigger>("generate_mesh", generate_mesh_cb);
  }
...
int main(int argc, char * argv[])
{
    ...
    node->declare_parameter("cols", rclcpp::ParameterValue(640));
    node->declare_parameter("rows", rclcpp::ParameterValue(480));
    node->declare_parameter("cx", rclcpp::ParameterValue(320.0));
    node->declare_parameter("cy", rclcpp::ParameterValue(240.0));
    node->declare_parameter("fx", rclcpp::ParameterValue(550.0));
    node->declare_parameter("fy", rclcpp::ParameterValue(550.0));
    node->declare_parameter("volume_x", rclcpp::ParameterValue(640));
    node->declare_parameter("volume_y", rclcpp::ParameterValue(640));
    node->declare_parameter("volume_z", rclcpp::ParameterValue(640));
    node->declare_parameter("volume_resolution", rclcpp::ParameterValue(0.002));
    node->declare_parameter("tsdf_frame", rclcpp::ParameterValue("tsdf_frame"));
    node->declare_parameter("bilateral_sigma_depth", rclcpp::ParameterValue(0.04f));
    node->declare_parameter("bilateral_sigma_spatial", rclcpp::ParameterValue(4.5f));
    node->declare_parameter("bilateral_kernel_size", rclcpp::ParameterValue(7));
    node->declare_parameter("tsdf_min_camera_movement", rclcpp::ParameterValue(0.f));
    node->declare_parameter("tsdf_trunc_dist");
    node->declare_parameter("tsdf_max_weight", rclcpp::ParameterValue(50));
    node->declare_parameter("raycast_step_factor", rclcpp::ParameterValue(0.25));
    node->declare_parameter("gradient_delta_factor", rclcpp::ParameterValue(0.25));
    node->declare_parameter("light_pose_x", rclcpp::ParameterValue(0.f));
    node->declare_parameter("light_pose_y", rclcpp::ParameterValue(0.f));
    node->declare_parameter("light_pose_z", rclcpp::ParameterValue(0.f));
    node->declare_parameter("update_via_sensor_motion", rclcpp::ParameterValue(false));

    std::string tsdf_base_frame;
    node->get_parameter("tsdf_frame", tsdf_base_frame);

    node->get_parameter("update_via_sensor_motion", kinfu_params.update_via_sensor_motion);  // deprecated?

    node->get_parameter("fx", kinfu_params.intr.fx);
    node->get_parameter("fy", kinfu_params.intr.fy);
    node->get_parameter("cx", kinfu_params.intr.cx);
    node->get_parameter("fy", kinfu_params.intr.cy);

    float world_to_vol_x, world_to_vol_y, world_to_vol_z;
    node->get_parameter("kinfu_params.world_to_volume.x", world_to_vol_x);
    node->get_parameter("kinfu_params.world_to_volume.y", world_to_vol_y);
    node->get_parameter("kinfu_params.world_to_volume.z", world_to_vol_z);
    ...
    node->get_parameter("volume_x", voxels_x);
    node->get_parameter("volume_y", voxels_y);
    node->get_parameter("volume_z", voxels_z);

    float volume_resolution;
    node->get_parameter("volume_resolution", volume_resolution);

    // TODO: Autocompute resolution from volume length/width/height in meters
    kinfu_params.volume_dims = cv::Vec3i(voxels_x, voxels_y, voxels_z);
    kinfu_params.volume_resolution = volume_resolution;
    kinfu_params.volume_pose = Eigen::Affine3f::Identity(); // This gets overwritten when Yak is initialized
    ...
    double trunc_dist;
    node->get_parameter_or("tsdf_trunc_dist", kinfu_params.tsdf_trunc_dist, 5.0f * volume_resolution); //meters;
    node->get_parameter("tsdf_max_weight", kinfu_params.tsdf_max_weight);
    node->get_parameter("raycast_step_factor", kinfu_params.raycast_step_factor);
    node->get_parameter("gradient_delta_factor", kinfu_params.gradient_delta_factor);

    RCLCPP_INFO(node->get_logger(), "Starting fusion node");

    auto fusion = std::make_shared<Fusion>(node, kinfu_params, tsdf_base_frame);
